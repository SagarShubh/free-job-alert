...
export default function EditJob() {
    ...
    useEffect(() => {
        const fetchJob = async () => {
            ...
            // 2. If not (first time loading a draft), try to parse from the "flat" columns the crawler filled.
            ...
            // AI Fallback hydration (if relational is empty but flat columns have data)
            if (job.status === 'draft' && !job.job_fees?.length && job.application_fee) {
                // Crawler stores simple text, so we might just put it in the first box or leave empty for manual entry.
                // Or if it stored JSON, we parse it. For now, we assume user manually transfers complicated text.
                // But wait, the crawler engine DOES try to put structured data if possible.
            }
            ...
            if (job.status === 'draft' && !job.job_links?.length && job.important_links) {
                try {
                    ...
                    const parsed = typeof job.important_links === 'string' ? JSON.parse(job.important_links) : job.important_links;
                    if (Array.isArray(parsed)) {
                        links = parsed.map((l: any) => ({ link_title: l.title || 'Link', url: l.url || '' }));
                    }
                } catch (e) { }
            }
    ...
    const handleArrayChange = (section: string, index: number, field: string, value: string) => {
        setFormData(prev => {
            const updated = [...(prev as any)[section]];
    ...
    const addItem = (section: string, template: any) => {
        setFormData(prev => ({ ...prev, [section]: [...(prev as any)[section], template] }));
    };
